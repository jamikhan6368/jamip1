// lib/automation.js - V112: COMPLETE CODE (UI + AUTOMATION ENGINE)
...
(() => {
  ...
  function createFloatingUI() {
      ...
      style.textContent = \`
        ...
        /* UI ELEMENTS */
        .app-shell { padding: 20px; display: flex; flex-direction: column; gap: 15px; }
        ...
        .section-label { font-size: 11px; text-transform: uppercase; letter-spacing: 0.08em; color: var(--text-muted); display: flex; align-items: center; gap: 10px; margin-bottom: 5px; font-weight: 700; }
        ...
        .styled-select { background: var(--bg-input); color: var(--text-main); border: 1px solid var(--border-color); border-radius: 6px; padding: 4px; font-size: 11px; margin-right: 6px; outline: none; }
        .main-input { width: 100%; background: var(--bg-input); border: 1px solid var(--border-color); color: var(--text-main); padding: 10px; border-radius: 8px; font-size: 12px; margin-bottom: 5px; outline: none; transition: 0.2s; }
        ...
        .btn-primary { background: var(--gradient-primary); color: white; width: 100%; padding: 14px; font-size: 13px; text-transform: uppercase; letter-spacing: 0.5px; border-radius: 8px; box-shadow: 0 4px 15px rgba(14, 165, 233, 0.4); margin-top: 10px; border: none; cursor: pointer; }
      ...
      STATE.targets.forEach(t => {
          const trainBtn = document.querySelector(\`.btn-train[data-target="\${t}"]\`);
          if(trainBtn) trainBtn.onclick = () => startTraining(t);
          const testBtn = document.querySelector(\`.btn-test[data-target="\${t}"]\`);
          if(testBtn) testBtn.onclick = () => testTarget(t);
      });

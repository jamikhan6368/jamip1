// lib/automation.js - V113: FULL APP + PERSISTENCE FIX
module.exports = `
(() => {
  ...
  console.log("Runway Pro: Loaded from Server");
  ...
  function createFloatingUI() {
      ...
      // NOTE: Backticks are escaped with backslash (\\) to prevent server crash
      style.textContent = \`
        ...
        #runway-pro-panel {
            ...
            display: none; flex-direction: column; box-shadow: 0 30px 80px rgba(0,0,0,0.9);
        ...
        .btn-primary { background: linear-gradient(135deg, #0ea5e9, #3b82f6); color: white; width: 100%; padding: 14px; font-size: 13px; font-weight: 700; margin-top: 10px; }
      ...
      panel.innerHTML = \`
        <div class="app-shell">
          <div class="header">
            <div><div class="brand-title">Runway Automation Pro</div><div style="font-size:10px; color:#94a3b8;">Powered by Moin Datori</div></div>
            <div class="header-right" style="display:flex; gap:8px;">
                ...
                <div onclick="document.getElementById('runway-pro-panel').classList.remove('show')" style="cursor:pointer; padding:5px; font-size:16px;">✕</div>
            </div>
          </div>
  ...
  async function performClick(targetData, timeout = 3000, isDoubleClick = false) {
      ...
      el.style.outline = "3px solid #d946ef"; await sleep(150);
